...
class chaoyang_dataloader():  
    ...
    def run(self,mode,annotator,pred=[],prob=[],paths=[]):        
        if mode=='warmup':
            warmup_dataset = chaoyang_dataset(self.root,transform=self.transform_train, mode='all', annotator=annotator)
            ...
            labeled_dataset = chaoyang_dataset(self.root,transform=self.transform_train, mode='labeled',pred=pred, probability=prob,paths=paths, annotator=annotator)
            labeled_loader = DataLoader(
                dataset=labeled_dataset, 
                batch_size=self.batch_size,
                shuffle=True,
                num_workers=self.num_workers)           
            unlabeled_dataset = chaoyang_dataset(self.root,transform=self.transform_train, mode='unlabeled',pred=pred, probability=prob,paths=paths, annotator=annotator)
            ...
            eval_dataset = chaoyang_dataset(self.root,transform=self.transform_test, mode='all', annotator=annotator)
